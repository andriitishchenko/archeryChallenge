...
- Mobile browser viewport → opts into the edge-to-edge safe-area viewport, fills the dynamic device viewport, and uses manipulation touch handling → the SPA occupies the mobile/tablet viewport without double-tap zoom while scene scrolling remains available.
