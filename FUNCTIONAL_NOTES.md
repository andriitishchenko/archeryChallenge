...
- Starting or switching to a match → resets the local arrow input, shared status message, and opponent live-result indicator, then restores only the selected match's server state and same-match live arrow series → a parallel or rematch session cannot display scores or waiting text carried over from the previously displayed match.
