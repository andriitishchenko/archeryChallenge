...
- Starting or switching to a match → resets the local arrow input and opponent live-result indicator, then restores only the selected match's server state → a parallel or rematch session cannot display scores carried over from the previously displayed match.
