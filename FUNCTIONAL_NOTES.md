...
- Bot/offline fallback → loads the current shadow-bot helper before the scoring UI and adapts live arrows and round totals to the player's current series, completed bot rounds, historical average, and skill profile, while varying round outcomes → cached script versions cannot leave scoring without `botShadowShoot`, the completion view re-renders the exact final bot array used for scoring, a run such as 10–10–10 is mirrored near 10 on the bot side, and the bot remains local-only rather than persisted server data.
