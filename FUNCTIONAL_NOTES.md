...
- Bot/offline fallback → runs a shadow bot that adapts live arrows and round totals to the player's current series, completed bot rounds, historical average, and skill profile, while varying round outcomes → a run such as 10–10–10 is mirrored near 10 on the bot side and the bot remains local-only rather than persisted server data.
